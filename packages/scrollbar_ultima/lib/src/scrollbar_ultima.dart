// packages\scrollbar_ultima\lib\src\scrollbar_ultima.dart
...
class ScrollbarUltimaState extends State<ScrollbarUltima>
  ...
  // RTL fix for horizontal scrollbars (top/bottom)
  bool get _isHorizontalRtl =>
      !isVerticalScroll && Directionality.of(context) == TextDirection.rtl;
  ...
  void _onThumbDragged(DragUpdateDetails details) {
    ...
    final rawDelta = details.primaryDelta ?? 0.0;
    final dragDelta = _isHorizontalRtl ? -rawDelta : rawDelta;
  ...
  void _onTrackTappedOrDragged(Offset position) {
    ...
    final localAxis = isVerticalScroll ? position.dy : position.dx;
    final effectiveAxis =
        _isHorizontalRtl ? (_thumbScrollRange - localAxis) : localAxis;

    final absolute =
        (effectiveAxis / _thumbScrollRange).clamp(0.0, 1.0);

    final newOffset = (((_maxScrollOffset! - _minScrollOffset) * absolute) +
            _minScrollOffset)
        .clamp(_minScrollOffset, _maxScrollOffset!);
