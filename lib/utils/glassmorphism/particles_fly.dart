...
void connectLines() {
  lineOffset = [];
  const maxDist = 260.0; // طول الخط (كبّره/صغّره)

  for (int i = 0; i < offsets.length; i++) {
    for (int j = i + 1; j < offsets.length; j++) { // مهم: بدون تكرار
      final dx = offsets[j].dx - offsets[i].dx;
      final dy = offsets[j].dy - offsets[i].dy;
      final dist = sqrt(dx * dx + dy * dy);

      if (dist <= maxDist) {
        lineOffset.add([offsets[i], offsets[j], dist]);
      }
    }
  }
}
...
for (var item in lineOffsets) {
  final dist = item[2] as double;
  const maxDist = 260.0;

  final t = (1 - (dist / maxDist)).clamp(0.0, 1.0); // 1 قريب، 0 بعيد

  final p = Paint()
    ..color = lineColor.withOpacity(0.08 + (0.35 * t)) // وضوح حسب القرب
    ..strokeWidth = lineStrokeWidth * (0.2 + (0.8 * t)); // سمك حسب القرب

  canvas.drawLine(item[0] as Offset, item[1] as Offset, p);
}
