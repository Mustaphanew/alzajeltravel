...
class NotificationModel {
  ...
  String titleByLang(String langCode) => (langCode == 'ar')
      ? (titleAr.isNotEmpty ? titleAr : titleEn)
      : (titleEn.isNotEmpty ? titleEn : titleAr);

  String bodyByLang(String langCode) => (langCode == 'ar')
  ...
  factory NotificationModel.fromDbMap(Map<String, Object?> row) {
    ...
    if (ca is int)
      createdAt = ca;
    else if (ca is BigInt)
      createdAt = ca.toInt();
    else
      createdAt = int.tryParse(ca?.toString() ?? '') ?? 0;
  ...
  factory NotificationModel.fromFcmData(
    ...
    final createdAt =
        int.tryParse((data['created_at'] ?? '').toString()) ??
        DateTime.now().millisecondsSinceEpoch;

    final titleEn = (data['title_en'] ?? data['title'] ?? 'Notification')
        .toString();
